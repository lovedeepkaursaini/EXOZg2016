...
//    double rangeLo=TMath::Max(600., massL-0.3*massL), rangeHi=TMath::Min(3600., massL+0.3*massL);
      double rangeLo=TMath::Max(600., massL-5*0.05*massL), rangeHi=TMath::Min(3600., massL+5*0.05*massL);
                       ...
                       bool focus=false)
{
    ...
    lumi_13TeV  = "4 fb^{-1}"; // default is "19.7 fb^{-1}"
...
//histZg_minitree_mrgd_ZToQQ_W_0-p-014_M_1750.root
        ...
        TH1D *h_mX_SR;//=(TH1D*)file->Get("distribs_2_10_0");
        if(postfix=="antibtag")h_mX_SR =(TH1D*) h_mX_ab->Clone();
        else if(postfix=="btag")h_mX_SR = (TH1D*)h_mX_b->Clone();

        std::cout<< h_mX_SR->GetName()<<std::endl;
        ...
        plot_vg->SetTitle("");
        plot_vg->GetYaxis()->SetRangeUser(0.01, 1000);
